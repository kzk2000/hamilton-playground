# FROM https://staggrid-examples.streamlit.app/
...
def fetch_data(samples):
    ...
    dummy_data = {
        ...
        "apple":np.random.randint(0,100,samples) * 10000,
        "banana":np.random.randint(0,100,samples) / 5.0,
        "chocolate":np.random.randint(0,100,samples)*100000,
...
sample_size = st.sidebar.number_input("rows", min_value=5, value=30)
grid_height = st.sidebar.number_input("Grid height", min_value=20, max_value=800, value=300)
...
gb.configure_column("chocolate", type=["numericColumn", "numberColumnFilter", "customCurrencyFormat"], custom_currency_symbol="US$ ", aggFunc='max')
...
with st.spinner("Displaying results..."):
    ...
    st.markdown("""
        All grid configuration is done through a dictionary passed as ```gridOptions``` parameter to AgGrid call.
